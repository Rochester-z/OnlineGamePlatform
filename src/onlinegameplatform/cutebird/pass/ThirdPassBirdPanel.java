package onlinegameplatform.cutebird.pass;        //包名根目录为src源文件下一级目录

import onlinegameplatform.cutebird.tool.BirdBullet;                    //导包根目录为src源文件下一级目录
...
public class ThirdPassBirdPanel extends JPanel implements MouseMotionListener, MouseListener {
    ...
    //墙体
    Image image2 = PictureUtil.getPictureBird("wall3.png").getImage();
    //小猪
...
//        g2.drawLine(shootx,shooty,122,449);
    ...
    public void paint(Graphics g){
        ...
        //绘制墙体
        g.drawImage(image2,0, 2, 777,500, null);
...
//            g.drawImage(image1, 0, 420, 150, 120, null);
        ...
        if(flag2) {            //当释放鼠标时,每隔10ms,小鸟子弹数组中的子弹移动一次位置,并画出来
    ...
    public void begin() throws InterruptedException {
        ...
        cuteBird3.setx(600);
        cuteBird3.sety(408);
        CuteBird cuteBird4 = new CuteBird();
        cuteBird4.setx(600);
        cuteBird4.sety(408);
        ...
        for(int i=0; i<2; i++){
            ...
            cuteBird.setx(400+100*i);
            cuteBird.sety(350+20*i);
            ListCuteBird.add(cuteBird);
        }
        for(int i=0; i<3; i++){
            CuteBird cuteBird = new CuteBird();
            cuteBird.setx(300-100*i);
            cuteBird.sety(300-20*i);
            ListCuteBird.add(cuteBird);
        }

        for(int i=0; i<2; i++){
            CuteBird cuteBird = new CuteBird();
            cuteBird.setx(500+50*i);
            cuteBird.sety(100+100*i);
            ListCuteBird.add(cuteBird);
        }
...
//        System.out.println("点击的x坐标"+x+"点击的y坐标"+y);
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        int x = e.getX();
        int y = e.getY();
        //绳子发射坐标必须在发射范围内，当鼠标释放的点在发射范围外时，取发射范围内最大点为绳子（拖动）发射坐标
