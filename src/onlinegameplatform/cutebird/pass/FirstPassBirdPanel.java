...
public class FirstPassBirdPanel extends JPanel implements MouseMotionListener, MouseListener {
    JFrame jf;   //将开始界面传入面板,来操作开始界面

    StageUI stageUI;  //将关卡选择界面传入得分界面来操作
    ...
    //墙体
    Image image2 = PictureUtil.getPictureBird("wall.png").getImage();
    //小猪
...
//        g2.drawLine(shootx,shooty,122,449);
    ...
    public void paint(Graphics g){
        ...
        //绘制墙体
        g.drawImage(image2, 600, 500, 200,300, null);
...
//            g.drawImage(image1, 0, 420, 150, 120, null);
        ...
        //绘制小鸟子弹
        //每隔10ms移动一次小鸟数组中子弹,重绘一次
        if(flag2) {            //当释放鼠标时,每隔10ms,小鸟子弹数组中子弹移动一次位置,并画出来
            for (int i = 0; i < ListBirdBullet.size(); i++) {
                ...
                System.out.println("cuteBird.x" + cuteBird.getx() + "cuteBird.y" + cuteBird.gety());
            }
        }
        ...
        cuteBird.move();
        cuteBird.drawbird(g);
//        g.drawImage(image3, cuteBird.getx(), cuteBird.gety(), 70,70,null);
